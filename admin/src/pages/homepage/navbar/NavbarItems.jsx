...
function NavbarItems() {
  ...
  const [isLoadingHero, setIsLoadingItems] = useState([]);
  ...
  const handleSearch = (event) => {
    ...
    if (allHeros) {
      const filtered = allHeros.filter((item) =>
        `${item.title} ${item.url}`.toLowerCase().includes(value)
      );
  ...
  const fetchNavbarItems = async () => {
    ...
    try {
      ...
      const companyRes = api.get("/api/navbar/");
  ...
  const handleRowClick = (navbaritem_id) => {
    navigate(`/homepage/navbar/edit/${navbaritem_id}`);
  };
          ...
          <div className="d-flex justify-content-between align-items-center mb-3">
            ...
            <div>
              ...
              <button
                ...
                onClick={() => navigate("/homepage/navbar/add")}
              >
                + Add Items
              </button>
            </div>
              ...
              {isLoading || isLoadingHero ? (
                ...
                paginatedData.map((item) => (
                  ...
                  >
                    ...
                    <td>{item.url}</td>
                    <td>{item.order}</td>
                    <td>{item.dropdown_parent_data?.name || "NA"}</td>
                    <td>
                      {item.status ? (
